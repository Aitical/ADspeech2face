...
def load_face(face_item):
    face_data = Image.open(face_item['filepath']).convert('RGB')
    face_lr = face_data.resize((16, 16))
    face_lr = np.transpose(np.array(face_lr), (2, 0, 1)).astype('float32')
    face_data = face_data
...
class FaceDataset(Dataset):
    ...
    def __getitem__(self, index):
        face_data, face_label, face_lr = load_face(self.face_list[index])
        if np.random.random() > 0.5:
            face_data = np.flip(face_data, axis=2).copy()
            face_lr = np.flip(face_lr, axis=2).copy()
        return face_data, face_label, face_lr
