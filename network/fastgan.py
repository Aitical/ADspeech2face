...
class Generator(nn.Module):
    def __init__(self, ngf=64, nz=100, nc=3, im_size=1024):
        ...
        self.to_64 = conv2d(nfc[64], nc, 3, 1, 1, bias=False)
...
if __name__ ==  '__main__':
    g_model = Generator(im_size=128, nz=64)
    d_model = Discriminator(im_size=128)
    a = torch.rand(4, 3, 128, 128)
    d_loss , (rec1, rec2, rec3), p = d_model(a, label='real')
    print(d_loss.shape, rec1.shape, rec2.shape, rec3.shape, p)
    g_out = g_model(torch.rand(2, 64, 1, 1))
    print(g_out[0].shape, g_out[1].shape)
