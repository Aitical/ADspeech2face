...
for it in range(150000):
    ...
    fake_score_out, fake_rec = d_net(fake.detach())
    ...
    D_arcface_loss = l2_loss(F.normalize(real_rec_embd, dim=1), F.normalize(fake_rec_embd, dim=1))
    ...
    (G_contrastive_loss + 0.3*reconstruction_loss + arcface_loss + 0.2*loss32).backward()
